...
void Ckol12022lampaView::OnDraw(CDC* pDC)
{
	...
	CDC* mem8DC = new CDC();
	mem8DC->CreateCompatibleDC(pDC);
	CRect r;
	GetClientRect(&r);

	CBitmap memBitmap;
	memBitmap.CreateCompatibleBitmap(pDC, r.Width(), r.Height());
	mem8DC->SelectObject(&memBitmap);

	// Fill the memory DC with white background
	mem8DC->FillSolidRect(0, 0, r.Width(), r.Height(), RGB(255, 255, 255));


	auto oldGM = mem8DC->SetGraphicsMode(GM_ADVANCED);
	XFORM oldForm;
	mem8DC->GetWorldTransform(&oldForm);

	DrawLamp(mem8DC, false);

	mem8DC->SetWorldTransform(&oldForm);
	mem8DC->SetGraphicsMode(oldGM);

	
	pDC->BitBlt(0, 0, r.Width(), r.Height(), mem8DC, 0, 0, SRCCOPY);
...
BOOL Ckol12022lampaView::OnEraseBkgnd(CDC* pDC)
{
	// TODO: Add your message handler code here and/or call default
