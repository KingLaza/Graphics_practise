...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
	...
	switch (nChar) {
	case VK_LEFT:
		m_glRenderer.m_CamBeta += rotAngle;
		break;
	case VK_RIGHT:
		m_glRenderer.m_CamBeta -= rotAngle;
		break;
	case VK_UP:
		m_glRenderer.m_CamAlpha -= rotAngle;
		break;
	case VK_DOWN:
		m_glRenderer.m_CamAlpha += rotAngle;
		break;
	case VK_ADD:
	case 187:
		m_glRenderer.m_CamDist += zoomStep;
		break;
	case VK_SUBTRACT:
	case 189:
		m_glRenderer.m_CamDist -= zoomStep;
	...
	/*m_glRenderer.m_CamDist = Clamp(m_glRenderer.m_CamDist, 8, 50);

	m_glRenderer.m_CamAlpha = Clamp(m_glRenderer.m_CamAlpha, 1, 180);*/
